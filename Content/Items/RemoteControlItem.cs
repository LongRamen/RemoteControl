...
public class RemoteControlItem : ModItem
{
    ...
    private static MethodInfo HitWireSingle;
    ...
    public override void Load()
    {
        ...
        HitWireSingle = typeof(Wiring).GetMethod("HitWireSingle", BindingFlags.NonPublic | BindingFlags.Static);
    }
    ...
    {
        if (player.whoAmI != Main.myPlayer || player.JustDroppedAnItem) return true;

        if (Main.netMode != NetmodeID.Server && !ModLoader.HasMod("TerrariaOverhaul"))
        {
            On |= Common.GlowmaskHelper.EditGlowMask(glowMaskID, glowmaskTextureOn);
        }
        ...
        {
            HitWireSingle.Invoke(null, [pos.X, pos.Y]); // Common.WiringHelper.HitWireSingle(pos.X, pos.Y);
        }
        else
        {
